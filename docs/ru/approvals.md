...
Сброс runtime откладывается на **500 ms**, запись атомарная. После перезапуска демона одобрения и блокировки читаются из `runtime.yaml`. Просроченные записи отбрасываются при компиляции снимка.
