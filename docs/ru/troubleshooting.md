...
`--socket` у клиента хука и у демона должен совпадать. Устаревший сокет удаляется только при старте под блокировкой. Операционные логи демона: `agentd.log` в [каталоге состояния](./configuration.md#каталог-состояния).
