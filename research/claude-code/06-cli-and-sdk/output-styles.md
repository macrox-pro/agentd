...
>
>   **Create a Markdown file**: Save it at one of three levels. The file name becomes the style name unless you set `name` in the frontmatter.
>
...
>
>   **Add frontmatter and instructions**: Decide whether to keep Claude Code's software engineering instructions. Set `keep-coding-instructions: true` if you're changing how Claude communicates but still want it coding the same way. Leave it out if Claude won't be doing software engineering.
>
...
>     ```
>
>   **Switch to your style**: Run `/config` in the terminal and select your style under **Output style**, or set `outputStyle` in a settings file to the style's name. It takes effect after `/clear` or the next time you start a session.
>
